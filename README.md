...
## CyberMythos: Authorized Linux Security Extension

This fork adds **CyberMythos**, a safety-scoped extension for Linux-focused security learning, CTF labs, bug-bounty reporting, internal authorized penetration testing, and defensive validation. CyberMythos keeps the OpenMythos recurrent-depth transformer architecture and adds domain presets, safety policy checks, Linux workflow helpers, and a CLI example.

> CyberMythos is intended only for systems you own, administer, or have explicit permission to test. It refuses or redirects requests involving credential theft, persistence, stealth, destructive behavior, or unauthorized compromise.
...
```python
from open_mythos.cyber import CyberMythos, LinuxSecurityAssistant, summarize_command_risks

assistant = LinuxSecurityAssistant(mode="ctf")
print(assistant.ctf_workflow("linux privilege lab", "linux"))
print(summarize_command_risks(["nmap -sV 10.10.10.5"], authorization_confirmed=True))
...
| File | Purpose |
| --- | --- |
| `open_mythos/cyber.py` | CyberMythos presets, safety policy, Linux helper workflows, and wrapper class. |
| `examples/cyber_mythos_cli.py` | CLI for command risk classification, CTF workflows, recon plans, reports, and hardening. |
